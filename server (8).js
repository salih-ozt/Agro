...
app.post('/api/auth/register', registerLimiter, validateAuthInput, upload.single('profilePic'), async (req, res) => {
    try {
        ...
        if (req.file) {
            ...
            try {
                await sharp(req.file.path, { limitInputPixels: MAX_IMAGE_PIXELS, sequentialRead: true }).rotate().resize(300, 300, { fit: 'cover' }).webp({ quality: 55, effort: 1 }).toFile(outputPath);
    ...
    try {
        ...
        await Promise.all([
            (async () => {
                ...
                const outputPath = path.join(profilesDir, filename);
                await sharp(file.path, { sequentialRead: true, limitInputPixels: MAX_IMAGE_PIXELS })
                    .rotate()
        ...
        if (hasMedia) {
            for (let fi = 0; fi < req.files.length; fi++) {
                ...
                if (isVideo) {
                    ...
                    try {
                        const info = await sharp(file.path, { sequentialRead: true, limitInputPixels: MAX_IMAGE_PIXELS })
                            .rotate()
    ...
    try {
        ...
        if (req.file) {
            ...
            if (isVideo) {
                ...
                await sharp(req.file.path, { limitInputPixels: MAX_IMAGE_PIXELS, sequentialRead: true }).rotate().resize(1080, 1920, { fit: 'inside', withoutEnlargement: true }).webp({ quality: 85 }).toFile(dest);
                await fs.unlink(req.file.path).catch(() => {});
                mediaUrl = `/uploads/posts/${filename}`;
            }
        }
...
app.post('/api/messages/image', authenticateToken, upload.single('image'), async (req, res) => {
    try {
        ...
        await sharp(req.file.path, { limitInputPixels: MAX_IMAGE_PIXELS, sequentialRead: true }).rotate().resize(1920,1920,{fit:'inside',withoutEnlargement:true}).webp({quality:85}).toFile(outPath);
        await fs.unlink(req.file.path).catch(()=>{});
...
app.post('/api/chats/group', authenticateToken, upload.single('photo'), async (req, res) => {
    try {
        ...
        if (req.file) {
            ...
            await sharp(req.file.path, { limitInputPixels: MAX_IMAGE_PIXELS, sequentialRead: true }).rotate().resize(256,256,{fit:'cover'}).webp({quality:85}).toFile(out);
...
app.post('/api/hasat-takip/tarlalar/:id/foto', authenticateToken, upload.single('foto'), async (req, res) => {
    try {
        ...
        const dest = path.join(postsDir, filename);
        await sharp(req.file.path, { limitInputPixels: MAX_IMAGE_PIXELS, sequentialRead: true }).rotate().resize(1080, 1080, { fit:'inside', withoutEnlargement:true }).webp({ quality:82 }).toFile(dest);
        await require('fs').promises.unlink(req.file.path).catch(()=>{});
    ...
    (async () => {
        try {
            ...
            server.listen(PORT, '0.0.0.0', async () => {
                console.log(`
╔══════════════════════════════════════════════════╗
║  🌾 AGROLINK SERVER - PostgreSQL v7.2             ║
